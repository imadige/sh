...
class CustomergroupsController extends Controller
{
	...
	{
		$model=new Customergroups('search');
		$model->unsetAttributes();  // clear any default values
		$model->companyID=Yii::app()->user->getState("companyID");
		$model->worldID=Yii::app()->user->getState("worldID");
		$model->deleted=1;
		$model->worldID=Yii::app()->user->getState("worldID");
		if(isset($_GET['Customergroups']))
			$model->attributes=$_GET['Customergroups'];
	 ...
	 * @param Customergroups $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='customergroups-form')
		{
