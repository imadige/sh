...
class Warehouse extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, companyID, worldID, deleted', 'required'),
			array('companyID, worldID, deleted', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>25),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('warehouseID, name, companyID, worldID, deleted', 'safe', 'on'=>'search'),
		);
	}
