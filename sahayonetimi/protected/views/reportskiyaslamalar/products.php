...
<div class="reports">
  <div class="kiyastop clearfix">
  	...
  	<?php $this->widget('zii.widgets.jui.CJuiAutoComplete', array(
			...
			'source'=>$this->createUrl('products/getproducts'),
			'htmlOptions'=>array(
				'size'=>'50'
			),
		)); 
...
function raporla(){
	if(dataItem.length<1){
		...
		$('.reports_Ki').html('<div class="loading"><img src="<?=Yii::app()->baseUrl?>/images/loading.gif" /></div>');
		 $.post("<?=Yii::app()->createUrl("reportskiyaslamalar/products_")?>",{data:data_},function(data){
